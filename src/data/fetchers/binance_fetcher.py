...
def map_binance_interval(tf_input: str) -> str | None:
    ...
    if not BINANCE_AVAILABLE: return None # Should not be called if lib missing

    tf_input_upper = tf_input.upper()
    # Mapping from MQL5 style to Binance constants
    mapping = {
    ...
    # Get all valid interval constants from the BinanceClient class
    valid_binance_intervals = [getattr(BinanceClient, attr) for attr in dir(BinanceClient) if attr.startswith('KLINE_INTERVAL_')]

    if tf_input_upper in mapping:
        return mapping[tf_input_upper]
    elif tf_input in valid_binance_intervals: # Check if input is already a valid constant string
        return tf_input
    else:
        logger.print_error(f"Invalid Binance timeframe input: '{tf_input}'. Use M1, H1, D1 etc. or Binance KLINE_INTERVAL_* strings.")
        return None
...
def fetch_binance_data(ticker: str, interval: str, start_date: str, end_date: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        # Make end date inclusive by adding one day and subtracting 1 millisecond
        ...
        while attempt < max_attempts_per_chunk and not success:
            ...
            try:
                ...
                if status_code == 429: # Rate limit
                    ...
                    time.sleep(wait_time)
                elif status_code == 400 and getattr(e, 'code', '') == -1121: # Invalid symbol
