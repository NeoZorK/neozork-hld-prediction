...
Usage: --rule stochoscillator(14,3,3) or --rule stochoscillator(14,3,3,close)
...
def calculate_stochoscillator(df: pd.DataFrame, k_period: int = 14, d_period: int = 3, 
    ...
    if len(df) < max(k_period, d_period, slowing):
        logger.print_warning(f"Not enough data for Stochastic Oscillator calculation. Need at least {max(k_period, d_period, slowing)} points, got {len(df)}")
...
def calculate_stochoscillator_signals(k_values: pd.Series, d_values: pd.Series, 
                                     overbought: float = 80, oversold: float = 20) -> pd.Series:
    """
    Calculate trading signals based on Stochastic Oscillator levels.
...
def apply_rule_stochoscillator(df: pd.DataFrame, point: float, 
                               k_period: int = 14, d_period: int = 3, slowing: int = 3,
                               overbought: float = 80, oversold: float = 20,
                               price_type: PriceType = PriceType.CLOSE):
    """
    Applies Stochastic Oscillator rule logic to calculate trading signals and price levels.
    ...
    Returns:
        pd.DataFrame: DataFrame with Stochastic Oscillator calculations and signals
    """
    open_prices = df['Open']
    
    # Calculate Stochastic Oscillator
    k_values, d_values = calculate_stochoscillator(df, k_period, d_period, slowing, price_type)
    
    df['StochOsc_K'] = k_values
    df['StochOsc_D'] = d_values
    
    # Add price type info to column name
    price_name = "Open" if price_type == PriceType.OPEN else "Close"
    df['StochOsc_Price_Type'] = price_name
    
    # Calculate Stochastic Oscillator signals
    df['StochOsc_Signal'] = calculate_stochoscillator_signals(k_values, d_values, overbought, oversold)
    
    # Calculate support and resistance levels based on Stochastic Oscillator
    volatility_factor = 0.02  # 2% volatility assumption
    
    # Support level (when Stochastic Oscillator is oversold, expect price to bounce up)
    support_levels = open_prices * (1 - volatility_factor)
    
    # Resistance level (when Stochastic Oscillator is overbought, expect price to fall)
    ...
    df['PColor2'] = SELL
    df['Direction'] = df['StochOsc_Signal']
