...
class EDAAnalyzer:
    ...
    def run_duplicates_analysis(self, system) -> bool:
        """
        Run detailed duplicates analysis on all preloaded data:
        - Main timeframe dataset
        - Each file in multi-timeframe datasets
        ...
        print("-" * 40)
        df = system.current_data
        main_dupe_summary = self._analyze_duplicates(df)
        ...
        if hasattr(system, 'other_timeframes_data') and system.other_timeframes_data:
            for timeframe, timeframe_data in system.other_timeframes_data.items():
                if timeframe_data is not None and not timeframe_data.empty:
                    ...
                    # Note: We could add more detailed counting here if needed
        ...
        if total_duplicates > 0:
            print(f"\n💡 RECOMMENDATIONS:")
            print(f"   • Consider using 'Fix Data Issues' option to remove duplicates")
