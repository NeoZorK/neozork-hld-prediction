...
class Subscription:
    ...
    def __post_init__(self):
        ...
        if self.status == SubscriptionStatus.TRIALING and not self.trial_start:
            ...
            self.trial_end = datetime.now(timezone.utc) + timedelta(days=14)
    ...
    def reset_period_usage(self) -> None:
        ...
        if self.billing_cycle == "annual":
            self.current_period_end = datetime.now(timezone.utc) + timedelta(days=365)
        elif self.billing_cycle == "quarterly":
            self.current_period_end = datetime.now(timezone.utc) + timedelta(days=90)
        else:  # monthly
    ...
    def cancel(self, effective_date: Optional[datetime] = None) -> None:
        ...
        self.cancelled_at = effective_date or datetime.now(timezone.utc)
