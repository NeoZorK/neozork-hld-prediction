#!/bin/bash
      ...
      if [[ $(uname -m) == "arm64" ]]; then
          ...
          act -j build -P "${ACT_RUNNER_IMAGE_MAPPING}" --container-architecture linux/amd64 --container-options "${CONTAINER_OPTS}" "${ENV_VARS[@]}" --bind --container-daemon-socket unix:///var/run/docker.sock
      else
