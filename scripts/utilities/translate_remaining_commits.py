...
def translate_message(message):
    ...
    replacements = [
        ...
        (r'\\bдобавлен HTML-интерфейс\\b', 'add HTML interface'),
        ...
        (r'\\bс рабочим кодом от идеи до деплоя\\b', 'with working code from idea to deployment'),
        (r'\\bДобавлены все компоненты\\b', 'Added all components'),
        (r'\\bмодели\\b', 'models'),
        (r'\\bиндикаторы\\b', 'indicators'),
        (r'\\bблокчейн-интеграция\\b', 'blockchain integration'),
        (r'\\bРеализована система автоматического переобучения\\b', 'Implemented automatic retraining system'),
        (r'\\bДобавлен полный мониторинг\\b', 'Added full monitoring'),
        (r'\\bсистема алертов\\b', 'alert system'),
        (r'\\bСоздана документация\\b', 'Created documentation'),
        (r'\\bна запуску\\b', 'on launch'),
        ...
        (r'\\bПростой пример\\b', 'Simple example'),
        (r'\\bСложный пример\\b', 'Advanced example'),
