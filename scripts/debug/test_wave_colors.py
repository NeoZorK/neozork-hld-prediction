...
def test_wave_colors():
    ...
    try:
        ...
        if '_Plot_Color' in result.columns:
            ...
            if set(unique_colors) == expected_colors:
                print("✅ All expected colors present: Black, Red, Blue")
            else:
                missing = expected_colors - set(unique_colors)
                print(f"⚠️  Missing colors: {missing}")
        ...
        print(f"❌ Error testing Wave indicator colors: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """Main function."""
    print("🚀 Starting Wave indicator color test...")
    
    success = test_wave_colors()
    
    if success:
        print("\n✅ Wave indicator color test completed successfully!")
        print("🎯 Color fixes implemented:")
        print("   - Wave line now shows Black (NOTRADE=0) for no signal")
        print("   - Wave line now shows Red (BUY=1) for buy signals")
        print("   - Wave line now shows Blue (SELL=2) for sell signals")
        print("   - Signals only display when _Signal == 1 (BUY) or _Signal == 2 (SELL)")
    else:
        print("\n❌ Wave indicator color test failed!")
        return 1
