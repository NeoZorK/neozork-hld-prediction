...
def create_mock_args(mode='binance', ticker='TICKER', interval='h1', start='2023-01-10', end='2023-01-11', point=0.01, **kwargs):
...
class TestDataAcquisitionCaching(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_print_success = patch('src.data.data_acquisition.print_success')
        ...
        self.mock_print_success = self.patcher_print_success.start()
    ...
    def tearDown(self):
        ...
        self.patcher_print_success.stop()
    ...
    def test_get_interval_delta(self):
        self.patcher_print_warning.stop() # Test real function warning
        try:
            ...
            self.assertEqual(_get_interval_delta('4h'), pd.Timedelta(hours=4))
             ...
             self.patcher_print_warning.start()
    ...
    def test_csv_mode_calls_fetch_csv(self):
        ...
        # --- Define simple mock return value for stat ---
        mock_stat_result = MagicMock()
        mock_stat_result.st_size = 512

        # --- Apply SIMPLE mocks using return_value ---
        # No side_effect functions defined here anymore
        # Patch exists globally to just return True for this test's scope
        # Patch stat globally to just return the mock result for this test's scope
        # This is simpler but less precise than side_effect if other paths were checked
        with patch.object(Path, 'stat', return_value=mock_stat_result), \
             patch.object(Path, 'exists', return_value=True):

            # print("DEBUG (test): Running acquire_data inside SIMPLE mocked context...")
            data_info = acquire_data(args)
            # print("DEBUG (test): acquire_data finished.")
        # --- End mocking ---

        # Assert fetch_csv_data was called correctly
        ...
        # print(f"DEBUG (test): Final error message in data_info: {data_info['error_message']}")
        self.assertIsNone(data_info['error_message']) # Check error message is None
        pd.testing.assert_frame_equal(data_info['ohlcv_df'], mock_df_csv)
        self.assertEqual(data_info['file_size_bytes'], 512) # Check mocked file size
