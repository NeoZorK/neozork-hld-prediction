# tests/data/fetchers/test_csv_fetcher.py # FINAL CORRECTIONS V2
...
"""
        ...
        expected_df = pd.DataFrame(expected_data, index=expected_dates, dtype=np.float64) # Use float
        ...
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            ...
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.read_csv(StringIO(csv_content), sep=',', header=1, skipinitialspace=True)):
                ...
                self.assertIsNotNone(df) # Check it's not None
                self.assertEqual(len(df), 2)
                self.assertIsInstance(df.index, pd.DatetimeIndex)
                self.assertTrue(all(col in df.columns for col in ['Open', 'High', 'Low', 'Close', 'Volume']))
                df.index.name = 'DateTime' # Ensure name consistency
                # Compare only expected columns, ignore potential extra ones in df
...
"""
        ...
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            ...
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.read_csv(StringIO(csv_content), sep=',', header=1, skipinitialspace=True)):
                ...
                df.index.name = 'DateTime' # Ensure name for comparison
