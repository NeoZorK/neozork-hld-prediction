# tests/data/fetchers/test_csv_fetcher.py # CORRECTED Assertions & Logic
...
"""
        ...
        expected_dates = pd.to_datetime(["2023-01-01 10:00", "2023-01-01 10:01", "2023-01-01 10:02"])
        expected_data = {
            'Open': [1.1000, 1.1001, 1.1012],
            'High': [1.1050, 1.1061, np.nan], # inf becomes NaN
            'Low': [1.0950, 1.0961, 1.0972],
            'Close': [1.1000, 1.1011, np.nan], # -inf becomes NaN
            'Volume': [1000.0, 1100.0, 1200.0],
            'Predicted_High': [1.1060, 1.1071, 1.1082],
            'AnotherCol': [np.nan, 123.0, 45.6] # Coerced to numeric
        }
...
""" # Header exists but no data
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            ...
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.read_csv(StringIO(csv_content), sep=',', header=1, skipinitialspace=True)):
                 ...
                 self.assertIsNone(df) # Becomes None after dropna
...
0,"2023-01-01 10:00",1.1,1.1,1.1,1.1,100 # Wrong format -> NaN DateTime
