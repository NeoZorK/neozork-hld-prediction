# tests/data/fetchers/test_csv_fetcher.py # FINAL CORRECTIONS
...
"""
        # Expected DataFrame after cleaning (Row 2 dropped due to NaN in High/Close after inf replacement)
        expected_dates = pd.to_datetime(["2023-01-01 10:00", "2023-01-01 10:01"], name='DateTime')
        expected_data = {
            'Open': [1.1000, 1.1001], 'High': [1.1050, 1.1061], 'Low': [1.0950, 1.0961],
            'Close': [1.1000, 1.1011], 'Volume': [1000.0, 1100.0],
            'Predicted_High': [1.1060, 1.1071], 'AnotherCol': [np.nan, 123.0]
        }
        expected_df = pd.DataFrame(expected_data, index=expected_dates)
        expected_df.index.name = 'DateTime' # Set index name for comparison
        ...
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            ...
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.read_csv(StringIO(csv_content), sep=',', header=1, skipinitialspace=True)):
                ...
                # Compare resulting frame (allow extra columns in actual if needed)
                pd.testing.assert_frame_equal(df[expected_df.columns], expected_df)
    ...
    def test_fetch_csv_data_empty_file(self, _):
        # CORRECTED: Simulate read_csv returning an empty DataFrame
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            mock_path_instance = MockPath.return_value
            mock_path_instance.is_file.return_value = True
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.DataFrame()):
                 df = fetch_csv_data("dummy/empty.csv")
                 self.assertIsNone(df) # Function returns None if read gives empty
...
"""
        expected_dates = pd.to_datetime(["2023-01-01 10:01"], name='DateTime')
        expected_data = {
        ...
        expected_df.index.name = 'DateTime' # Set index name
        ...
        with patch('src.data.fetchers.csv_fetcher.Path') as MockPath:
            ...
            with patch('src.data.fetchers.csv_fetcher.pd.read_csv', return_value=pd.read_csv(StringIO(csv_content), sep=',', header=1, skipinitialspace=True)):
                ...
                pd.testing.assert_frame_equal(df, expected_df) # Compare resulting frame
