# tests/data/fetchers/test_polygon_fetcher.py # FINAL CORRECTIONS V7 (PEP8 Fix for mock arg name)
...
# We no longer need to mock the client here as we patch the source library directly
if POLYGON_AVAILABLE:
    try:
        from polygon.rest.models import Agg
        # We still need BadResponse from the original library for type hints/checks
        # Keep OriginalBadResponse name to avoid conflict with variable name below
        from polygon.exceptions import BadResponse as OriginalBadResponse
    except ImportError:
        class Agg:
            # Rename 'open' parameter to 'open_' to avoid shadowing built-in
            def __init__(self, timestamp=None, open_=None, high=None, low=None, close=None, volume=None):
                 self.timestamp=timestamp
                 # Assign to 'self.open_'
                 self.open_=open_
                 self.high=high
                 self.low=low
                 self.close=close
                 self.volume=volume
        class OriginalBadResponse(Exception): # Dummy Exception
            pass
else:
    class Agg:
        # Rename 'open' parameter to 'open_' to avoid shadowing built-in
...
# Use the imported/dummy BadResponse for consistent type checking in tests
# Rename variable to avoid conflict with class if BadResponse is used directly later
...
class TestPolygonFetcher(unittest.TestCase):
    ...
    # Rename MockRESTClient -> mock_rest_client
    ...
    def test_resolve_polygon_ticker_success_currency(self, mock_rest_client, mock_sleep, __):
        ...
        # Use the imported/dummy BadResponse
    ...
    def test_fetch_polygon_data_success_single_chunk(self, mock_resolve, mock_rest_client, mock_getenv, _):
        ...
        agg1 = Agg(timestamp=1672531200000, open_=1.1, high=1.11, low=1.09, close=1.1, volume=1000.0) # Use open_
        agg2 = Agg(timestamp=1672617600000, open_=1.11, high=1.12, low=1.1, close=1.11, volume=1100.0) # Use open_
        mock_client_instance.get_aggs.return_value = [agg1, agg2]

        expected_dates = pd.to_datetime([1672531200000, 1672617600000], unit='ms')
        # Ensure column names here match the final DataFrame column names
        ...
        # Rename MockRESTClient -> mock_rest_client
        mock_rest_client.assert_called_once_with(api_key="fake_api_key")
    ...
    def test_fetch_polygon_data_success_pagination(self, mock_resolve, mock_rest_client, mock_getenv, mock_sleep, _):
        ...
        agg1 = Agg(timestamp=1711929600000, open_=70000, high=70100, low=69900, close=70050, volume=10.0) # Use open_
        agg2 = Agg(timestamp=1711929660000, open_=70050, high=70200, low=70000, close=70150, volume=11.0) # Use open_
        agg3 = Agg(timestamp=1712016000000, open_=70150, high=70300, low=70100, close=70250, volume=12.0) # Use open_
        ...
        # Check result DataFrame columns (should be Open, High, etc.)
    ...
    def test_fetch_polygon_data_chunk_api_error_429_retry(self, mock_resolve, mock_rest_client, mock_getenv, mock_sleep, _):
        ...
        agg1 = Agg(timestamp=1711929600000, open_=70000, high=70100, low=69900, close=70050, volume=10) # Use open_
        ...
        self.assertIsNotNone(result_df) # Check if None after fixing patch target
        if result_df is not None: # Only check length if not None
            self.assertEqual(len(result_df), 1)
        self.assertEqual(mock_client_instance.get_aggs.call_count, 3)
        self.assertEqual(mock_sleep.call_count, 5) # Keep corrected assertion
