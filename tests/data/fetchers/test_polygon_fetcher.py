# tests/data/fetchers/test_polygon_fetcher.py # FINAL CORRECTIONS V3
...
class TestPolygonFetcher(unittest.TestCase):
    ...
    def test_resolve_polygon_ticker_success_first_try(self, MockRESTClient, _, __):
        ...
        client_instance = MockRESTClient() # Instantiate the mock
    ...
    def test_resolve_polygon_ticker_success_currency(self, MockRESTClient, mock_sleep, __):
        ...
        # Correctly create BadResponse with a message string
        error_404_1 = BadResponse("Ticker EURUSD not found.")
        setattr(error_404_1, 'response', mock_response_404)
        # Subsequent call succeeds
        mock_client.get_ticker_details.side_effect = [ error_404_1, MagicMock() ] # Success on C:EURUSD
        ...
        # Sleep is called after the first 404 attempt
    ...
    def test_resolve_polygon_ticker_fail_not_found(self, MockRESTClient, mock_sleep, __):
        ...
        mock_client.get_ticker_details.side_effect = error_404 # Raise 404 for all attempts
        ...
        # Expect sleep to be called after each 404
    ...
    def test_resolve_polygon_ticker_api_error_non_404(self, MockRESTClient, _, __):
        ...
        mock_response_401.json.side_effect = ValueError("No JSON") # Simulate non-JSON response
        mock_response_401.text = "Unauthorized"
        error_401 = BadResponse("Unauthorized")
        setattr(error_401, 'response', mock_response_401)
        mock_client.get_ticker_details.side_effect = error_401 # Raise 401 on first try

        client_instance = MockRESTClient()
        resolved = resolve_polygon_ticker("ANYTICKER", client_instance)
        self.assertIsNone(resolved) # Should fail on non-404 error
    ...
    def test_fetch_polygon_data_success_single_chunk(self, mock_resolve, MockRESTClient, mock_getenv, __):
        ...
        mock_resolve.return_value = "C:EURUSD" # Simulate successful resolution
        ...
        self.assertEqual(args_call['timespan'], "day") # Check timespan mapping
    ...
    def test_fetch_polygon_data_success_pagination(self, mock_resolve, MockRESTClient, mock_getenv, mock_sleep, __):
        ...
        # Simulate M1 interval over a date range that fits in one chunk
        # The test setup doesn't force date-based pagination for M1 here
        mock_client_instance.get_aggs.side_effect = [
             [agg1, agg2, agg3], # Return all 3 in the first call for the date range
             [] # Next call would be after end_date
        ]

        start_date = "2024-04-01"
        end_date = "2024-05-01" # One month range for M1 fits in one chunk request typically
        result_df = fetch_polygon_data("BTCUSD", "M1", start_date, end_date) # Use M1

        self.assertIsNotNone(result_df)
         # ***** CORRECTED ASSERTION *****
         # Based on the mock and M1 chunk logic, only one call to get_aggs is made
         # for this date range, returning all 3 results.
        self.assertEqual(len(result_df), 3)
        # Only one actual data fetch call expected
        self.assertEqual(mock_client_instance.get_aggs.call_count, 1)
        # No sleep between chunks as only one chunk is fetched
    ...
    def test_fetch_polygon_data_resolve_fails(self, mock_resolve, MockRESTClient, mock_getenv, __):
        mock_getenv.return_value = "fake_key"
        mock_resolve.return_value = None # Simulate resolution failure
        mock_client_instance = MockRESTClient.return_value
        result_df = fetch_polygon_data("FAIL", "D1", "2023-01-01", "2023-01-02")
        self.assertIsNone(result_df)
        mock_resolve.assert_called_once_with("FAIL", mock_client_instance)
        mock_client_instance.get_aggs.assert_not_called() # Should not reach get_aggs
    ...
    def test_fetch_polygon_data_chunk_api_error_429_retry(self, mock_resolve, MockRESTClient, mock_getenv, mock_sleep, __):
        ...
        # Simulate 2 fails (429) then 1 success for the single chunk attempt
        mock_client_instance.get_aggs.side_effect = [
        ...
        # Check call count (2 fails + 1 success for the single chunk)
        self.assertEqual(mock_client_instance.get_aggs.call_count, 3)
         # ***** CORRECTED ASSERTION *****
         # The 429 handler calls time.sleep(60) twice.
         # The inter-chunk sleep is not called as the loop ends.
         # The generic retry sleep `time.sleep(5 * attempt)` is *not* called when 429 occurs.
        self.assertEqual(mock_sleep.call_count, 2) # Expect 2 sleeps (one after each 429)
