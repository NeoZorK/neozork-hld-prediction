...
class TestStochIndicator:
    ...
    def test_stoch_with_invalid_periods(self):
        ...
        with pytest.raises(ValueError, match="Stochastic k_period must be positive"):
    ...
    def test_stoch_docstring_info(self):
        ...
        assert "Stochastic" in self.stoch.__name__
        assert "oscillators" in self.stoch.__name__.lower()
        assert len(self.stoch.__doc__) > 0
    ...
    def test_stoch_overbought_oversold_levels(self):
        ...
        overbought_k = k_values_overbought.mean()
        oversold_k = k_values_oversold.mean()
