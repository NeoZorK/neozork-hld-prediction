...
**Почему важна оптимизация моделей for продакшена?** Потому что продакшен предъявляет совершенно другие требования к производительности, памяти and скорости.
...
**🔧 Детальное description параметров оптимизации моделей for продакшена:**
...
- **parameters**:
 ...
 - **Тип**: str
 - **description**: Название столбца with целевой переменной
...
- **`label`**: Название целевой переменной
 - **Тип**: str
 - **description**: Столбец with целевой переменной for предсказания
- **`problem_type`**: Тип задачи машинного обучения
 - **Тип**: str
 - **Значения**: 'auto', 'binary', 'multiclass', 'regression'
 - **on умолчанию**: 'auto'
 - **description**: AutoGluon автоматически определяет тип задачи
...
- **parameters**:
 ...
 - **description**: Обученная модель AutoGluon
 - **`model_name`**: Имя for сохранения модели
 - **Тип**: str
 - **description**: Путь and имя for сохранения сжатой модели
- **Возвращаемое значение**: float - размер модели in MB

**parameters predictor.save():**
- **`model_name`**: Имя модели
 - **Тип**: str
 - **description**: Путь for сохранения модели
...
**🔧 Детальное description параметров валидации модели for продакшена:**
...
**🔧 Детальное description параметров FastAPI сервера:**

**FastAPI application:**
- **`title`**: Название API
 - **Тип**: str
 - **description**: Отображается in документации Swagger
 - **Рекомендации**: Use описательное название
- **`version`**: Версия API
 - **Тип**: str
 - **description**: Версия API for отслеживания изменений
...
**🔧 Детальное description параметров Docker контейнеризации:**
...
- **`CMD ["python", "app.py"]`**: Команда Launchа
 - **description**: Launchает application при старте контейнера
...
**🔧 Детальное description параметров Docker Compose:**

**Версия Docker Compose:**
- **`Version: '3.8'`**: Версия формата файла
 - **description**: Использует формат Docker Compose версии 3.8
 - **Преимущества**: Поддержка новых функций and улучшенная совместимость

**Сервис autogluon-api:**
- **`build: .`**: Сборка образа
 - **description**: Собирает образ из Dockerfile in текущей директории
...
- **`image: nginx:alpine`**: Готовый образ Nginx
 - **description**: Использует Alpine Linux версию Nginx
...
- **`depends_on`**: dependencies
 - **`autogluon-api`**: Nginx Launchается после API сервиса

**Сервис redis:**
- **`image: redis:alpine`**: Готовый образ Redis
 - **description**: Использует Alpine Linux версию Redis
...
- **`spec.selector.matchLabels`**: Селектор подов
 - **description**: Выбирает поды with соответствующими метками
- **`spec.template`**: Шаблон пода
 - **description**: Определяет конфигурацию подов

**Контейнер:**
- **`name: autogluon-api`**: Имя контейнера
 - **description**: Уникальное имя контейнера in поде
- **`image: autogluon-api:latest`**: Образ контейнера
 - **description**: Docker образ for Launchа
 - **Рекомендации**: Use конкретные теги версий вместо latest
- **`ports.containerPort: 8000`**: Порт контейнера
 - **description**: Порт, который слушает application
...
- **`readinessProbe`**: check готовности
 - **description**: Проверяет, готов ли контейнер принимать трафик
...
- **`kind: Service`**: Тип ресурса
 - **description**: Создает сервис for доступа к подам
- **`spec.selector`**: Селектор подов
 - **description**: Выбирает поды for балансировки нагрузки
- **`spec.ports`**: Проброс портов
 - **`port: 80`**: Внешний порт
 - **`targetPort: 8000`**: Порт контейнера
- **`type: LoadBalancer`**: Тип сервиса
 - **description**: Создает внешний LoadBalancer

**PersistentVolumeClaim:**
- **`kind: PersistentVolumeClaim`**: Тип ресурса
 - **description**: Запрос on постоянное хранилище
...
<img src="images/optimized/advanced_production_flow.png" alt="Monitoring and логирование" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 4: Система Monitoringа and логирования in продакшене*
...
**🔧 Детальное description параметров системы Monitoringа:**
...
- **parameters конструктора**:
 ...
 - **description**: Файл for записи логов Monitoringа

**Метод setup_logging():**
- **Назначение**: configuration системы логирования
- **parameters logging.basicConfig():**
 - **`level=logging.INFO`**: Уровень логирования
 - **Тип**: int
 - **Значения**: DEBUG(10), INFO(20), WARNING(30), ERROR(40), CRITICAL(50)
 - **description**: Логирует messages уровня INFO and выше
 - **`format`**: Формат логов
 - **Тип**: str
...
- **parameters**:
 - **`error`**: Исключение
 - **Тип**: Exception
 - **description**: Ошибка, которая произошла
...
- **Метрики**:
 ...
 - **Тип**: float
 - **Диапазон**: 0.0-100.0
 - **description**: Процент использования RAM
...
- **parameters конструктора**:
 ...
 - **Тип**: str
 - **description**: Адрес SMTP сервера for отправки email
 - **examples**: 'smtp.gmail.com', 'smtp.yandex.ru'
 - **`smtp_port`**: Порт SMTP сервера
 - **Тип**: int
 - **on умолчанию**: 587 (TLS), 465 (SSL)
 - **description**: Порт for подключения к SMTP серверу
 - **`email`**: Email отправителя
 - **Тип**: str
 - **description**: Email адрес for отправки уведомлений
 - **`password`**: Пароль email
 - **Тип**: str
 - **description**: Пароль for аутентификации on SMTP сервере
...
- **parameters**:
 ...
 - **Тип**: Dict[str, float]
 - **description**: Словарь with текущими значениями метрик
...
- **parameters конструктора**:
 ...
 - **description**: Количество параллельных потоков for обработки
...
- **parameters обработки**:
 ...
 - **description**: Количество запросов in одном батче
- **Особенности**:
...
- **parameters конструктора**:
 ...
 - **description**: Адрес Redis сервера for кэширования
 - **`redis_port`**: Порт Redis сервера
 - **Тип**: int
 - **on умолчанию**: 6379
 - **description**: Порт for подключения к Redis
 - **`ttl`**: Время жизни cache
 - **Тип**: int
 - **on умолчанию**: 3600 (1 час)
 - **Диапазон значений**: `[60, 86400]` (1 minutesа - 24 часа)
 - **description**: Время in секундах, через которое кэш истекает
...
- **parameters**:
 ...
 - **description**: Паттерн for поиска ключей in Redis
