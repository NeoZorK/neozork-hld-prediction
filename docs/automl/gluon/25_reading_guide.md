...
graph TD
 ...
 D --> D2[Неделя 2: Оценка and валидация]
...
**Фокус:** installation and первый запуск
...
4. **Раздел 3** - Продвинутая configuration
 ...
 - `holdout_frac`: Доля данных for holdout валидации
...
6. **Практика:** Создайте свою первую модель
 ...
 holdout_frac=0.2, # 20% for валидации
 verbosity=2
 )
 ```
...
**Время:** 2-3 часа in день
**Фокус:** Валидация and оценка качества
...
**Время:** 2-3 часа in день
**Фокус:** Переобучение and продвинутые техники

13. **Раздел 7** - Переобучение моделей
 - **parameters переобучения:**
...
4. **Раздел 5** - Валидация моделей
 - **parameters валидации for analysis:**
 - `num_bag_folds=10`: Больше фолдов for стабильности
 - `holdout_frac=0.3`: Больше данных for валидации
 - `auto_stack=True`: Автоматический стекинг
 - **Кросс-валидация:**
 ```python
 predictor.fit(
...
1. **Раздел 1** - Введение and installation
 - **parameters установки for торговли:**
 - `pip install autogluon.tabular[all]` - полная installation
 - `pip install yfinance ccxt` - данные with бирж
...
2. **Раздел 2** - Базовое использование
 ...
 eval_metric='f1', # F1 for торговли
 path='trading_model',
 verbosity=2,
...
6. **Раздел 22** - Супер-система
 - **parameters супер-системы for торговли:**
 - `ensemble_methods=['adaptive', 'context', 'temporal']`
...
1. **Раздел 1** - Введение and installation
 ...
 - CPU: 4+ ядра
 - Диск: 10GB+ (for данных and отчетов)

2. **Раздел 2** - Базовое использование
 - **parameters for бизнес-анализа:**
 ```python
 predictor = TabularPredictor(
 label='target',
 problem_type='binary',
...
**Цель:** Создать первую модель AutoML Gluon with пониманием всех параметров
...
**Цель:** Провести полную валидацию модели with пониманием всех параметров валидации
...
# create модели with параметрами валидации
predictor = TabularPredictor(
 label='target',
...
# Обучение with параметрами валидации
predictor.fit(
 train_data,
 time_limit=120, # Увеличенное время for лучшего качества
 presets='high_quality', # Высокое качество for валидации
 holdout_frac=0.2, # 20% данных for holdout валидации
...
# Кросс-валидация on обучающих данных
cv_scores = []
...
# Анализ производительности on классам
...
**Цель:** Создать полноценную продакшен API with пониманием всех параметров деплоя
...
# Глобальные переменные for мониторинга
request_count = 0
error_count = 0
...
# Декораторы for обработки ошибок and мониторинга
def handle_errors(f):
 @wraps(f)
...
def validate_input_data(data):
 ...
 raise ValueError("Данные должны быть списком or словарем")
...
@validate_api_key
def predict():
 """Основной endpoint for предсказаний"""
 ...
 # Получение and валидация данных
 try:
...
if __name__ == '__main__':
 logger.info(f"Запуск API сервера on {Config.HOST}:{Config.PORT}")
...
| **holdout_frac** | 0.2 | 0.2-0.3 | 0.1-0.2 | Доля данных for валидации |
...
1. **Начните with простого** - используйте parameters on умолчанию
