# AutoML Gluon - Complete guide пользователя
...
*Этот мануал содержит исчерпывающую информацию on всем аспектам работы with AutoML Gluon, from установки to продакшен деплоя, including специальную оптимизацию for Apple Silicon.*
...
- **Issues with производительностью**: Модели работают медленно or not работают вообще
...
- **Issues with GPU**: CUDA not работает, обучение идет только on CPU
...
**Почему TabularPredictor - это сердце AutoML Gluon?** Потому что он объединяет все возможности in одном простом интерфейсе. Это как универсальный пульт управления - одна кнопка Launchает сложные процессы.
...
**Goal:** Создать робастную продакшен систему
