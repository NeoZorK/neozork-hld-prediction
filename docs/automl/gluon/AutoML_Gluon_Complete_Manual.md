# AutoML Gluon - Полное guide пользователя
...
30. [Углубленное description методик создания and управления портфолио](./30_portfolio_management.md)
...
- **Масштабируемость**: Работает как on CPU, так and on GPU
- **Интеграция with AWS**: Легкая интеграция with облачными сервисами Amazon

## for кого этот мануал?

Этот мануал предназначен for:
- Data Scientists, которые хотят ускорить процесс creating ML models
- ML Engineers, работающих with продакшен системами
...
- **MLX интеграция** - использование Apple MLX фреймворка for ускорения
...
- **Мониторинг производительности** - отслеживание эффективности on Apple Silicon
...
- **Multi-Modal Learning** - работа with различными типами данных
...
*Этот мануал содержит исчерпывающую информацию on всем аспектам работы with AutoML Gluon, from установки to продакшен деплоя, включая специальную оптимизацию for Apple Silicon.*
...
**Почему 80% пользователей начинают with базового использования?** Потому что это самый простой способ понять, как работает AutoML Gluon. Это как обучение вождению - сначала изучаете основы, потом переходите к сложным маневрам.

### Что дает базовое понимание?
- **Быстрый старт**: from данных to модели за несколько строк кода
...
- **Фрустрация**: not понимаете, почему модель работает not так
...
*Рисунок 2: Архитектура AutoML Gluon with основными компонентами*

**Почему TabularPredictor - это сердце AutoML Gluon?** Потому что он объединяет все возможности in одном простом интерфейсе. Это как универсальный пульт управления - одна кнопка запускает сложные процессы.
...
- **Гибкость**: Работает with любыми табличными данными
...
**Почему используем 'auto'?** Потому что AutoML Gluon умнее нас in выборе оптимальных параметров.
...
- **Обработка ошибок**: Если столбец not найден, AutoML Gluon выдаст понятную ошибку

**parameter `problem_type`:**
- **Что означает**: Тип задачи машинного обучения
- **Зачем нужен**: Определяет, какие алгоритмы and метрики использовать
...
- **Преимущества автоматического определения**:
 ...
 - **Гибкость**: Работает with любыми данными
...
## Работа with различными типами данных
...
```python
# for текстовых колонок AutoGluon автоматически создает признаки
...
def training_callback(model_name, model_path, model_info):
 """Callback function for мониторинга обучения"""
...
- **Точность**: Модели работают on 10-30% лучше
...
- **Недоиспользование ресурсов**: GPU and CPU работают неэффективно
...
**Почему нужны кастомные гиперпараметры?** Потому что стандартные settings подходят for средних случаев, а ваши данные могут быть особенными.
...
**`feature_fraction`:**
- **Что означает**: Доля признаков, используемых for каждого дерева
...
**`bagging_fraction`:**
- **Что означает**: Доля данных, используемых for каждого дерева
- **Зачем нужен**: Предотвращает переобучение, увеличивает разнообразие
- **Рекомендуемые значения**:
...
predictor.fit(
 ...
 stack_ensemble_levels=[0, 1], # Какие уровни использовать for стекинга
...
# Ray планировщик for распределенного обучения
...
**Почему 70% ML-проектов терпят неудачу in продакшене из-за плохой валидации?** Потому что валидация - это единственный способ убедиться, что ваша модель действительно работает. Это как тест-драйв автомобиля перед покупкой.
...
- **Неправильные решения**: Выбираете плохую модель вместо хорошей
- **Бизнес-провалы**: Модель not работает in реальных условиях
...
- **Правильный выбор**: Выбираете лучшую модель for задачи
- **Уверенность in продакшене**: Модель действительно будет работать
...
**Почему Holdout валидация - самый простой and популярный метод?** Потому что она интуитивно понятна: вы просто разделяете данные on две части - одну for обучения, другую for проверки. Это как экзамен in школе - вы учитесь on учебнику, а экзамен сдаете on новым задачам.
...
**Почему K-Fold кросс-валидация более надежна, чем Holdout?** Потому что она использует ВСЕ данные and for обучения, and for валидации. Это как сдать 5 экзаменов вместо одного - результат будет более точным.

**Как работает K-Fold кросс-валидация:**
1. **Разделение on K частей**: Данные делятся on K равных частей (обычно K=5 or K=10)
...
5. **Усреднение результатов**: Итоговая оценка - среднее on всем K итерациям
...
- **not использовать**: Когда данные сбалансированы
...
**Почему 87% ML-моделей никогда not попадают in продакшен?** Потому что их создатели not понимают, что обучение модели - это только 20% работы. Остальные 80% - это подготовка к продакшену, мониторинг and поддержка.
...
**Почему продакшен in ML кардинально отличается from обычной разработки?** Потому что ML-модели - это not просто код, а живые системы, которые учатся and изменяются. Это как разница между заводом and садом - завод работает on плану, а сад требует постоянного ухода.
...
- **dependency from данных**: Нет данных = нет предсказаний
- **Черный ящик**: Сложно понять, почему модель приняла решение

Продакшен деплой ML-моделей - это критически важный этап, который требует тщательного планирования, мониторинга and поддержки. in этом разделе рассмотрим все аспекты деплоя AutoML Gluon моделей in продакшен.

## Подготовка модели к продакшену

### Оптимизация модели

**Почему модель, которая отлично работает in Jupyter, может провалиться in продакшене?** Потому что продакшен предъявляет совершенно другие требования к производительности, памяти and скорости.
...
def validate_production_model(predictor, test_data, performance_thresholds):
 ...
 # check стабильности предсказаний
...
### Аутентификация and авторизация
...
class PredictionCache:
 ...
 # remove наименее используемого элемента
...
def monitor_and_retrain():
 """Мониторинг and автоматическое переобучение"""
...
class MultiModelSystem:
 ...
 # Получение предсказаний from всех моделей
 Predictions = {}
 probabilities = {}
...
class SCHRShort3M1Analysis:
 """Анализ SCHR SHORT3 on 1-minutesном таймфрейме"""
...
class SCHRShort3M15Analysis:
 """Анализ SCHR SHORT3 on 15-minutesном таймфрейме"""
...
class SCHRShort3H4Analysis:
 """Анализ SCHR SHORT3 on 4-часовом таймфрейме"""
...
class SCHRShort3D1Analysis:
 """Анализ SCHR SHORT3 on дневном таймфрейме"""
...
class SCHRShort3W1Analysis:
 """Анализ SCHR SHORT3 on недельном таймфрейме"""
...
class SCHRShort3MN1Analysis:
 """Анализ SCHR SHORT3 on месячном таймфрейме"""
...
class SCHRShort3DEXIntegration:
 """Интеграция SCHR SHORT3 with DEX"""
...
**Почему 90% людей бросают изучение ML, not имея четкого плана?** Потому что они пытаются изучить все сразу, not понимая, with чего начать and как двигаться дальше. Это как попытка построить дом без чертежей.
...
**Почему новичкам нужен особый подход?** Потому что они еще not понимают основ ML and могут легко запутаться in сложных концепциях. Нужен пошаговый подход with быстрыми результатами.

### 🚀 Быстрый старт (1-2 недели)

**Почему быстрый старт критически важен for новичков?** Потому что они должны увидеть результаты как можно быстрее, чтобы not потерять мотивацию.

**Цель:** Запустить первый example как можно быстрее

#### День 1-2: Основы
1. **Раздел 1** - Введение and installation
2. **Раздел 2** - Базовое использование
3. **Практика:** Установите AutoML Gluon and запустите первый example
...
15. **Практика:** Создайте систему with переобучением
...
10. **Практика:** Задеплойте систему in продакшен

### 🔬 Углубленное изучение (2-3 недели)

**Цель:** Стать экспертом in AutoML Gluon
...
13. **Раздел 7** - Переобучение моделей
14. **Практика:** Задеплойте and оптимизируйте систему
