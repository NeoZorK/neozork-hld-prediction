...
*Рисунок 1: architecture AutoML Gluon with основными componentsи*
...
**Почему важно понимать components?** Потому что это помогает понять, как TabularPredictor автоматизирует весь process machine learning:
...
### 📊 comparison типов задач

<img src="images/optimized/metrics_Detailed.png" alt="comparison типов задач" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 4: Детальное comparison типов задач and их метрик*

**Почему важно сравнивать типы задач?** Потому что это помогает выбрать правильный подход and metrics for вашей задачи:
...
### 🔄 process обучения модели

<img src="images/optimized/retraining_workflow.png" alt="process обучения модели" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 5: Детальная схема процесса обучения модели in AutoML Gluon*

**Почему важно понимать process обучения?** Потому что это помогает понять, что происходит внутри AutoML Gluon and как оптимизировать обучение:
...
- **Что означает**: Доля данных for holdout validation (from 0.0 to 1.0)
- **Зачем нужен**: Создает отдельный набор данных for финальной оценки модели
- **on умолчанию**: `None` (используется кросс-validation)
- **Рекомендуемые значения**:
...
- **Стандартная validation**: `holdout_frac=0.2` (20%)
- **Тщательная validation**: `holdout_frac=0.3` (30%)
- **Когда использовать**:
...
*Рисунок 6: Различные Methods validation and оценки качества моделей*

**Почему важна правильная validation?** Потому что она обеспечивает надежность and качество моделей:
...
- **Зачем нужен**: Позволяет получить подробную информацию о performance
- **on умолчанию**: `False` (базовый Report)
- **Доступные значения**:
- **`False`** - Базовый Report (только основные metrics)
- **`True`** - Детальный Report (все доступные metrics)
- **Практические examples**:
...
- **Детальный Report**: Все доступные metrics + статистика
- **Когда использовать**:
...
- **Что означает**: Включать ли дополнительные metrics in оценку
- **Зачем нужен**: Позволяет получить расширенный набор метрик
- **on умолчанию**: `True` (включать дополнительные metrics)
- **Доступные значения**:
- **`False`** - Только основная метрика
- **`True`** - Основная + дополнительные metrics
- **Практические examples**:
...
**Оптимизация performance оценки:**
...
#### 🔧 Детальное describe параметров validation

**parameter `holdout_frac` - Holdout validation**

- **Что означает**: Доля данных, выделяемая for holdout validation (from 0.0 to 1.0)
- **Зачем нужен**: Создает отдельный набор данных for финальной оценки модели
- **on умолчанию**: `None` (используется кросс-validation)
- **Рекомендуемые значения**:
- **Малые data (< 1000 строк)**: `0.1-0.2` (10-20%)
...
- **Что означает**: Количество фолдов for бэггинга (bagging) validation
- **Зачем нужен**: Создает ансамбль моделей for повышения стабильности
- **on умолчанию**: `8` (8 фолдов)
- **Рекомендуемые значения**:
- **Быстрое обучение**: `3-5` фолдов
...
- **Что означает**: Количество уровней стекинга (stacking) validation
- **Зачем нужен**: Создает многоуровневые ансамбли for повышения качества
- **on умолчанию**: `0` (без стекинга)
- **Рекомендуемые значения**:
- **Быстрое обучение**: `0` (без стекинга)
...
```python
# Стратегия 1: Быстрая validation
predictor.fit(
 train_data,
 holdout_frac=0.2,
 num_bag_folds=3,
...
# Стратегия 2: Стандартная validation
predictor.fit(
 train_data,
 holdout_frac=0.2,
 num_bag_folds=5,
...
# Стратегия 3: Качественная validation
predictor.fit(
 train_data,
 holdout_frac=0.2,
 num_bag_folds=8,
...
**Оптимизация validation on размеру данных:**
...
**Влияние параметров validation on performance:**

```python
# Анализ влияния параметров
def analyze_validation_impact():
"""Анализ влияния параметров validation on время and качество"""
...
"""Выбор стратегии validation on basis требований"""

 if data_size < 1000:
# Малые data - простая validation
 return {
 'holdout_frac': 0.1,
 'num_bag_folds': 3,
 'num_bag_sets': 1,
...
# Средние data - сбалансированная validation
...
# Большие data - качественная validation
...
- **Когда использовать**:
- **Анализ and визуализация**: `as_pandas=True`
- **Высокая performance**: `as_pandas=False`
...
- **Когда использовать**:
- **comparison моделей**: `as_pandas=True`
...
*Рисунок 8: process Settings гиперпараметров and оптимизации*
...
**Почему важен Monitoring обучения?** Потому что он помогает контролировать process and выявлять проблемы:

- **Progress Tracking**: Отслеживание прогресса обучения
- **Performance Monitoring**: Monitoring performance моделей
...
- **Model Comparison**: comparison различных моделей
...
- **Validation Strategy**: Правильная стратегия validation
...
- **Monitoring**: Постоянный Monitoring performance
