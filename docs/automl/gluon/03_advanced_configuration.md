...
- **Что означает**: Какие уровни моделей использовать for создания стекинга
- **Зачем нужен**: Позволяет контролировать, какие модели участвуют in стекинге
...
*Рисунок 4: Оптимизация and управление ресурсами in AutoML Gluon*
...
- **Что означает**: Количество CPU ядер, используемых for обучения
...
- **Что означает**: Количество GPU, используемых for обучения
...
- **Влияние on производительность**:
 ...
 - **with GPU**: Быстрое обучение нейронных сетей
...
- **Зачем нужен**: Определяет, как разделять данные on обучающую and валидационную выборки
- **on умолчанию**: `'auto'` (автоматический выбор)
- **Доступные значения**:
...
- **parameters**:
 - **`n_splits`**: Количество фолдов (on умолчанию 5)
 - **`shuffle`**: Перемешивать ли данные (on умолчанию False)
...
def seasonal_validation(X, y, seasonal_period=12, n_splits=5):
 ...
 # Обучающая выборка: все данные to определенного момента
...
- **Зачем нужен**: Контролирует, какие типы признаков создавать and как их обрабатывать
...
- **Зачем нужен**: Преобразует категориальные данные in числовые
- **on умолчанию**: `True` (включено)
- **Типы кодирования**:
...
- **Когда использовать**:
 ...
 - **Отключить**: Когда временные данные not важны
...
def choose_feature_parameters(data_type, text_columns, categorical_columns, numeric_columns):
 ...
 # Текстовые данные - акцент on текстовые признаки
 return {
 'enable_text_special_features': True,
 'enable_text_ngram_features': True,
 'text_ngram_range': (1, 3),
 ...
 # Категориальные данные - акцент on категориальные признаки
 return {
 'enable_text_special_features': False,
 ...
 # Числовые данные - акцент on числовые признаки
 return {
 'enable_text_special_features': False,
 'enable_text_ngram_features': False,
 'enable_categorical_encoding': True,
 ...
 # Смешанные данные - сбалансированная configuration
 return {
 'enable_text_special_features': True,
 'enable_text_ngram_features': True,
 'text_ngram_range': (1, 3),
...
- **Когда использовать**:
 - **False**: for метрик, работающих with предсказаниями
 - **True**: for метрик, работающих with вероятностями
...
- **Что означает**: Список метрик for оценки качества модели
