...
*Рисунок 1: process Settings and оптимизации гиперпараметров*
...
'feature_fraction': 0.9, # Доля признаков for каждого дерева (предотвращает retraining)
'bagging_fraction': 0.8, # Доля данных for каждого дерева
'bagging_freq': 5, # Частота применения bagging
'min_data_in_leaf': 20, # Минимум данных in листе (предотвращает retraining)
...
'feature_fraction': 0.8, # Меньше признаков for предотвращения retraining
...
- **Много деревьев**: Медленное обучение, но может быть retraining
...
- **Зачем нужен**: Больше листьев = сложнее модель, но риск retraining
- **Рекомендуемые значения**:
- **Простые data**: `31-63`
- **Средние data**: `63-127`
...
- **Влияние on модель**:
- **Мало листьев**: Простая модель, меньше retraining
- **Много листьев**: Сложная модель, больше retraining
...
- **Зачем нужен**: Предотвращает retraining, увеличивает разнообразие
- **Рекомендуемые значения**:
- **Много признаков**: `0.5-0.8`
- **Среднее количество**: `0.8-0.9`
- **Мало признаков**: `0.9-1.0`
- **Влияние on модель**:
- **Маленькая доля**: Больше разнообразия, меньше retraining
- **Большая доля**: Меньше разнообразия, больше retraining
...
- **Зачем нужен**: Предотвращает retraining, стабилизирует модель
- **Рекомендуемые значения**:
- **Большие data**: `10-50`
- **Средние data**: `20-100`
- **Малые data**: `50-200`
- **Влияние on модель**:
- **Маленькое значение**: Более детальная модель, риск retraining
- **Большое значение**: Более обобщенная модель, меньше retraining
- **Оптимальное**: Зависит from размера данных and сложности задачи

**`lambda_l1` and `lambda_l2`:**
- **Что означают**: L1 and L2 регуляризация for предотвращения retraining
...
#### 🔧 Детальное describe параметров оптимизации гиперпараметров

**parameter `num_trials` - Количество попыток оптимизации**

- **Что означает**: Количество различных комбинаций гиперпараметров for тестирования
- **Зачем нужен**: Больше попыток = лучше качество, но дольше время
- **on умолчанию**: `10` (10 попыток)
- **Рекомендуемые значения**:
...
**parameter `scheduler` - Planировщик задач**

- **Что означает**: Алгоритм Planирования параллельных задач оптимизации
- **Зачем нужен**: Управляет параллельным выполнением поиска гиперпараметров
- **on умолчанию**: `'local'` (локальное выполнение)
- **Доступные значения**:
- **`'local'`** - Локальное выполнение on одном компьютере
- **`'ray'`** - Распределенное выполнение через Ray
- **`'dask'`** - Распределенное выполнение через Dask
- **Практические examples**:
- **Один компьютер**: `scheduler='local'`
- **Кластер with Ray**: `scheduler='ray'`
- **Кластер with Dask**: `scheduler='dask'`
- **Влияние on performance**:
- **local**: Медленнее, но проще in настройке
- **ray/dask**: Быстрее, но требует Settings кластера
- **Когда использовать**:
- **Разработка**: `scheduler='local'`
- **Продакшен on кластере**: `scheduler='ray'` or `scheduler='dask'`

**parameter `searcher` - Алгоритм поиска**

- **Что означает**: Алгоритм поиска оптимальных гиперпараметров
- **Зачем нужен**: Определяет, как искать лучшие комбинации параметров
- **on умолчанию**: `'auto'` (автоматический выбор)
- **Доступные значения**:
...
- **Стандартные задачи**: `searcher='auto'` or `searcher='bayes'`
- **Критически важные задачи**: `searcher='grid'`

**parameter `Space` - Определение пространства поиска**

- **Что означает**: Определяет диапазон значений for каждого гиперпараметра
- **Зачем нужен**: Ограничивает поиск in разумных пределах
- **Синтаксис**: `Space(min_value, max_value)` for числовых параметров
- **Практические examples**:
- **Целые числа**: `Space(50, 500)` for количества деревьев
- **Дробные числа**: `Space(0.01, 0.3)` for скорости обучения
- **Logsческие значения**: `[True, False]` for булевых параметров
- **Влияние on поиск**:
...
- **Рекомендации on настройке**:
- **Начните with широкого диапазона**: `Space(1, 1000)` for количества деревьев
- **Сужьте on basis результатов**: `Space(100, 300)` после первых экспериментов
- **Use логарифмический масштаб**: for параметров with экспоненциальным влиянием

**Практические examples Settings оптимизации:**

```python
# Быстрая оптимизация for экспериментов
quick_optimization = {
 'num_trials': 5,
 'scheduler': 'local',
 'searcher': 'random'
...
# Стандартная оптимизация for development
...
# Качественная оптимизация for продакшена
...
# Полная оптимизация for критически важных задач
full_optimization = {
 'num_trials': 100,
 'scheduler': 'ray',
 'searcher': 'grid'
...
**Monitoring процесса оптимизации:**

```python
# configuration with Monitoringом
def optimization_callback(trial, score, hyperparameters):
"""Callback for Monitoringа оптимизации"""
...
### 🤝 architecture ансамблей

<img src="images/optimized/advanced_production_flow.png" alt="architecture ансамблей" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 3: architecture and configuration ансамблей in AutoML Gluon*

**Почему важна правильная configuration ансамблей?** Потому что ансамбли могут улучшить качество модели on 5-15%:
...
- **Multi-Level Stacking**: Многоуровневые ансамбли for максимального качества
...
#### 🔧 Детальное describe параметров ансамблей

**parameter `num_bag_folds` - Количество фолдов for бэггинга**

- **Что означает**: Количество фолдов for создания ансамбля через бэггинг
- **Зачем нужен**: Создает разнообразие моделей for повышения стабильности
...
- **Зачем нужен**: Создает иерархические ансамбли for максимального качества
...
**parameter `stack_ensemble_levels` - Уровни for стекинга**

- **Что означает**: What уровни моделей использовать for создания стекинга
- **Зачем нужен**: Позволяет контролировать, What модели участвуют in стекинге
- **on умолчанию**: `[0]` (только базовые модели)
- **Доступные значения**:
...
**Оптимизация ансамблей on размеру данных:**
...
**Влияние параметров ансамблей on performance:**

```python
# Анализ влияния параметров
def analyze_ensemble_impact():
"""Анализ влияния параметров ансамблей on время and качество"""
...
**Рекомендации on выбору стратегии ансамблей:**

```python
# Рекомендации on выбору стратегии
def choose_ensemble_strategy(data_size, time_limit, quality_requirement):
"""Выбор стратегии ансамблей on basis требований"""

 if data_size < 1000:
# Малые data - простые ансамбли
 return {
 'num_bag_folds': 3,
 'num_bag_sets': 1,
 'num_stack_levels': 0,
...
# Средние data - сбалансированные ансамбли
...
# Большие data - качественные ансамбли
...
<img src="images/optimized/apple_silicon_optimization.png" alt="Management ресурсами" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 4: Оптимизация and Management ресурсами in AutoML Gluon*

**Почему важна правильная configuration ресурсов?** Потому что она определяет performance and эффективность обучения:

- **CPU Optimization**: Оптимизация использования процессорных ядер
- **GPU acceleration**: Ускорение обучения with помощью графических процессоров
- **Memory Management**: Management оперативной памятью
- **Parallel Processing**: Параллельная обработка задач
- **Resource allocation**: Распределение ресурсов между алгоритмами
- **Load Balancing**: Балансировка нагрузки между componentsи
...
#### 🔧 Детальное describe параметров ресурсов

**parameter `ag_args_fit` - Аргументы for обучения**

- **Что означает**: Словарь with параметрами ресурсов for обучения моделей
- **Зачем нужен**: Контролирует использование CPU, GPU, памяти and времени
- **on умолчанию**: `{}` (автоматическое определение)
- **Основные parameters**:
- **`num_cpus`**: Количество CPU ядер for обучения
- **`num_gpus`**: Количество GPU for обучения
- **`memory_limit`**: Лимит памяти in гигабайтах
- **`time_limit`**: Лимит времени in секундах

**parameter `num_cpus` - Количество CPU ядер**

- **Что означает**: Количество CPU ядер, Useых for обучения
- **Зачем нужен**: Ускоряет обучение за счет параллельных вычислений
- **on умолчанию**: `None` (автоматическое определение)
- **Рекомендуемые значения**:
- **Малые задачи**: `2-4` ядра
- **Стандартные задачи**: `4-8` ядер
- **Большие задачи**: `8-16` ядер
- **Максимальная performance**: `16+` ядер
- **Влияние on performance**:
- **Мало ядер**: Медленное обучение, но стабильная Working
- **Много ядер**: Быстрое обучение, но может быть нестабильно
- **Оптимальное**: 70-80% from доступных ядер
- **Практические examples**:
- **Разработка**: `num_cpus=4`
- **Продакшен**: `num_cpus=8-12`
- **Кластер**: `num_cpus=16+`

**parameter `num_gpus` - Количество GPU**

- **Что означает**: Количество GPU, Useых for обучения
...
- **Несколько GPU**: `2+`
- **Влияние on performance**:
- **Без GPU**: Медленное обучение нейронных networks
- **with GPU**: Быстрое обучение нейронных networks
- **Множественные GPU**: Еще быстрее, но сложнее configuration
- **Практические examples**:
- **Разработка**: `num_gpus=1`
- **Продакшен**: `num_gpus=1-2`
- **Кластер**: `num_gpus=4+`

**parameter `memory_limit` - Лимит памяти**
...
**parameter `time_limit` - Лимит времени**
...
- **Что означает**: Алгоритм Planирования параллельных задач
- **Зачем нужен**: Управляет распределением задач on ресурсам
- **on умолчанию**: `None` (автоматический выбор)
- **Доступные типы**:
...
- **Кластер with Ray**: `RayScheduler(num_cpus=32, num_gpus=4)`
- **Кластер with Dask**: `DaskScheduler(num_cpus=32, num_gpus=4)`

**Стратегии Settings ресурсов:**
...
**Оптимизация ресурсов on размеру данных:**
...
**Monitoring использования ресурсов:**

```python
# Monitoring ресурсов во время обучения
...
#### 🔧 Детальное describe Planировщиков

**LocalScheduler - Локальный Planировщик**

- **Что означает**: Planировщик for выполнения задач on одном компьютере
- **Зачем нужен**: Простое and эффективное Planирование for локальной разработки
- **parameters**:
...
- **Преимущества**: Простота Settings, стабильность
- **Недостатки**: Ограниченная масштабируемость

**RayScheduler - Ray Planировщик**

- **Что означает**: Planировщик for распределенного выполнения через Ray
- **Зачем нужен**: Масштабирование on кластеры for больших задач
- **parameters**:
...
- **Недостатки**: Сложность Settings, дополнительные dependencies

**DaskScheduler - Dask Planировщик**

- **Что означает**: Planировщик for распределенного выполнения через Dask
- **Зачем нужен**: Масштабирование on кластеры with Dask
- **parameters**:
- **`num_cpus`**: Общее количество CPU ядер
- **`num_gpus`**: Общее количество GPU
...
- **Преимущества**: integration with Dask экосистемой
- **Недостатки**: Сложность Settings, дополнительные dependencies

**Выбор Planировщика:**

```python
# Рекомендации on выбору Planировщика
def choose_scheduler(resource_type, cluster_available=False):
"""Выбор Planировщика on basis ресурсов"""
...
### 📊 Стратегии validation

<img src="images/optimized/validation_methods.png" alt="Стратегии validation" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 5: Различные стратегии validation and их применение*

**Почему важна правильная стратегия validation?** Потому что она обеспечивает надежность and качество моделей:
...
### Кастомные стратегии validation
...
#### 🔧 Детальное describe кастомных стратегий validation

**parameter `validation_strategy` - Стратегия validation**

- **Что означает**: Тип стратегии validation for обучения моделей
- **Зачем нужен**: Определяет, как разделять data on обучающую and валидационную выборки
- **on умолчанию**: `'auto'` (автоматический выбор)
- **Доступные значения**:
- **`'auto'`** - Автоматический выбор стратегии
- **`'holdout'`** - Holdout validation
- **`'kfold'`** - K-fold кросс-validation
- **`'custom'`** - Кастомная стратегия validation
- **Практические examples**:
- **Стандартные задачи**: `validation_strategy='auto'`
- **Временные ряды**: `validation_strategy='custom'`
- **Несбалансированные data**: `validation_strategy='custom'`

**parameter `custom_validation_strategy` - Кастомная стратегия**

- **Что означает**: function for создания кастомных разделений данных
- **Зачем нужен**: Позволяет реализовать специфические стратегии validation
- **on умолчанию**: `None` (not используется)
- **Требования к functions**:
- **Входные parameters**: `(X, y, n_splits)` or `(X, y)`
- **Возвращаемое значение**: Генератор кортежей `(train_idx, val_idx)`
- **Тип данных**: indexы должны быть numpy arrays or списками
- **Практические examples**:
- **Временные ряды**: `TimeSeriesSplit`
- **Стратифицированная**: `StratifiedKFold`
- **Групповая**: `GroupKFold`

**TimeSeriesSplit - Временная validation**

- **Что означает**: Стратегия validation for временных рядов
- **Зачем нужна**: Предотвращает утечку данных из будущего in прошлое
- **parameters**:
- **`n_splits`**: Количество фолдов (on умолчанию 5)
...
- **Когда использовать**:
- **Временные ряды**: Обязательно for временных данных
- **Последовательные data**: Когда порядок важен
- **Прогнозирование**: Когда нужно предсказывать будущее

**StratifiedKFold - Стратифицированная validation**

- **Что означает**: Стратегия validation with сохранением пропорций классов
- **Зачем нужна**: Обеспечивает равномерное распределение классов in фолдах
- **parameters**:
- **`n_splits`**: Количество фолдов (on умолчанию 5)
- **`shuffle`**: Перемешивать ли data (on умолчанию False)
- **`random_state`**: Семя for воспроизводимости
- **Практические examples**:
- **Несбалансированные data**: `StratifiedKFold(n_splits=5, shuffle=True)`
...
- **Когда использовать**:
- **Несбалансированные data**: Когда классы представлены неравномерно
- **Классификация**: for задач классификации
- **Малые data**: Когда нужно сохранить пропорции классов

**GroupKFold - Групповая validation**

- **Что означает**: Стратегия validation with учетом групп in данных
- **Зачем нужна**: Предотвращает утечку данных между группами
- **parameters**:
- **`n_splits`**: Количество фолдов (on умолчанию 5)
- **Практические examples**:
- **Пользовательские data**: `GroupKFold(n_splits=5)`
- **Временные группы**: `GroupKFold(n_splits=3)`
- **Когда использовать**:
- **Пользовательские data**: Когда есть группы пользователей
- **Временные группы**: Когда есть временные группы
- **Географические data**: Когда есть географические группы

**Практические examples кастомных стратегий:**

```python
# example 1: Временная validation
...
# example 2: Стратифицированная validation
...
# example 3: Групповая validation
def group_validation(X, y, groups, n_splits=5):
"""Групповая validation for группированных данных"""
...
# example 4: Кастомная стратегия with фильтрацией
def custom_filtered_validation(X, y, n_splits=5, filter_func=None):
"""Кастомная validation with фильтрацией данных"""
 from sklearn.model_selection import KFold

# Применяем фильтр, if specified
...
**Дополнительные стратегии validation:**

```python
# example 5: validation with учетом временных окон
def time_window_validation(X, y, window_size=100, step_size=50):
"""validation with временными окнами"""
 n_samples = len(X)
...
# example 6: validation with учетом сезонности
def seasonal_validation(X, y, seasonal_period=12, n_splits=5):
"""validation with учетом сезонности"""
 n_samples = len(X)

 for i in range(n_splits):
# Обучающая выборка: все data to определенного момента
...
# example 7: validation with учетом распределения
def distribution_validation(X, y, n_splits=5, target_distribution=None):
"""validation with учетом распределения целевой переменной"""
 from sklearn.model_selection import KFold

 if target_distribution is None:
# Use стандартную K-fold валидацию
 kf = KFold(n_splits=n_splits, shuffle=True, random_state=42)
 for train_idx, val_idx in kf.split(X):
 yield train_idx, val_idx
 else:
# Кастомная Logsка for поддержания распределения
...
"""Выбор стратегии validation on basis типа данных and задачи"""

 if data_type == 'time_series':
# Временные ряды - Use временную валидацию
 return {
 'validation_strategy': 'custom',
 'custom_validation_strategy': time_series_validation
 }
 elif problem_type == 'classification' and data_size < 1000:
# Небольшая классификация - Use стратифицированную валидацию
 return {
 'validation_strategy': 'custom',
 'custom_validation_strategy': stratified_validation
 }
 elif data_type == 'grouped':
# Группированные data - Use групповую валидацию
 return {
 'validation_strategy': 'custom',
 'custom_validation_strategy': group_validation
 }
 else:
# Стандартные data - Use автоматическую стратегию
 return {
...
print("configuration for временных рядов:", time_series_config)
print("configuration for классификации:", classification_config)
print("configuration for группированных данных:", grouped_config)
print("Стандартная configuration:", standard_config)
```
...
*Рисунок 6: process инженерии and генерации признаков*

**Почему важна правильная инженерия признаков?** Потому что качественные признаки могут улучшить качество модели on 10-20%:
...
- **Feature Interaction**: create взаимодействий между приsignми
...
#### 🔧 Детальное describe параметров генерации признаков

**parameter `feature_generator_kwargs` - Аргументы генератора признаков**

- **Что означает**: Словарь with параметрами for Settings генерации признаков
- **Зачем нужен**: Контролирует, What типы признаков создавать and как их обрабатывать
- **on умолчанию**: `{}` (стандартные Settings)
...
- **Что означает**: parameters for обработки текстовых данных
...
- **`text_ngram_range`**: Диапазон N-gram (on умолчанию (1, 3))
- **`text_max_features`**: Максимальное количество признаков (on умолчанию 10000)
- **`text_min_df`**: Минимальная частота документа (on умолчанию 2)
- **`text_max_df`**: Максимальная частота документа (on умолчанию 0.95)

**parameter `enable_text_special_features` - Специальные текстовые признаки**

- **Что означает**: Включает create специальных признаков из текста
- **Зачем нужен**: Создает дополнительные признаки for улучшения качества модели
- **on умолчанию**: `True` (включено)
- **Создаваемые признаки**:
- **Длина текста**: Количество символов
- **Количество слов**: Количество слов in тексте
- **Количество предложений**: Количество предложений
- **Количество заглавных букв**: Количество заглавных букв
- **Количество цифр**: Количество цифр in тексте
...
- **Отключить**: Когда текстовые признаки not важны

**parameter `enable_text_ngram_features` - N-gram признаки**

- **Что означает**: Включает create N-gram признаков из текста
- **Зачем нужен**: Создает признаки on basis последовательностей слов/символов
- **on умолчанию**: `True` (включено)
- **Создаваемые признаки**:
...
- **Отключить**: Когда порядок слов not важен

**parameter `text_ngram_range` - Диапазон N-gram**

- **Что означает**: Диапазон размеров N-gram for создания признаков
- **Зачем нужен**: Контролирует сложность текстовых признаков
- **on умолчанию**: `(1, 3)` (unigrams, bigrams, trigrams)
- **Рекомендуемые значения**:
...
**parameter `text_max_features` - Максимальное количество признаков**

- **Что означает**: Максимальное количество текстовых признаков
- **Зачем нужен**: Ограничивает количество признаков for предотвращения retraining
- **on умолчанию**: `10000` (10000 признаков)
- **Рекомендуемые значения**:
...
- **Много признаков**: Может быть retraining
- **Практические examples**:
...
**parameter `text_min_df` - Минимальная частота документа**

- **Что означает**: Минимальная частота появления термина in документах
- **Зачем нужен**: Отфильтровывает редкие термины
- **on умолчанию**: `2` (термин должен появиться минимум in 2 документах)
- **Рекомендуемые значения**:
- **Малые data**: `1-2` (включать редкие термины)
- **Стандартные data**: `2-5` (стандартная фильтрация)
- **Большие data**: `5-10` (строгая фильтрация)
- **Влияние on качество**:
...
**parameter `text_max_df` - Максимальная частота документа**

- **Что означает**: Максимальная частота появления термина in документах
- **Зачем нужен**: Отфильтровывает слишком частые термины
- **on умолчанию**: `0.95` (термин not должен появиться in 95% документов)
- **Рекомендуемые значения**:
- **Стандартные data**: `0.95` (стандартная фильтрация)
- **Большие data**: `0.90` (строгая фильтрация)
- **Очень большие data**: `0.85` (очень строгая фильтрация)
- **Влияние on качество**:
...
- **Что означает**: parameters for обработки категориальных данных
...
- **`categorical_encoding`**: Тип кодирования (on умолчанию 'auto')
- **`categorical_max_levels`**: Максимальное количество уровней (on умолчанию 100)

**parameter `enable_categorical_encoding` - Кодирование категориальных признаков**

- **Что означает**: Включает автоматическое кодирование категориальных признаков
- **Зачем нужен**: Преобразует категориальные data in числовые
- **on умолчанию**: `True` (включено)
- **Типы кодирования**:
- **One-hot encoding**: Создает бинарные признаки for каждого уровня
- **Label encoding**: Присваивает числовые метки уровням
- **Target encoding**: Кодирует on basis целевой переменной
- **Практические examples**:
- **Включить**: `enable_categorical_encoding=True`
- **Отключить**: `enable_categorical_encoding=False`
- **Когда использовать**:
- **Включить**: Когда есть категориальные data
- **Отключить**: Когда категориальные data уже закодированы

**parameter `categorical_encoding` - Тип кодирования**

- **Что означает**: Тип кодирования for категориальных признаков
...
- **Когда использовать**:
- **auto**: for большинства случаев
- **onehot**: for категориальных данных with малым количеством уровней
- **label**: for категориальных данных with большим количеством уровней

**parameter `categorical_max_levels` - Максимальное количество уровней**

- **Что означает**: Максимальное количество уровней for категориального приsign
- **Зачем нужен**: Предотвращает create слишком большого количества признаков
- **on умолчанию**: `100` (100 уровней)
- **Рекомендуемые значения**:
- **Малые data**: `10-50` уровней
- **Стандартные data**: `50-100` уровней
- **Большие data**: `100-500` уровней
- **Влияние on performance**:
...
- **Что означает**: parameters for обработки числовых данных
...
**parameter `enable_numeric_imputation` - Заполнение пропущенных значений**

- **Что означает**: Включает автоматическое заполнение пропущенных значений
- **Зачем нужен**: Обрабатывает пропущенные значения in числовых данных
- **on умолчанию**: `True` (включено)
- **methods заполнения**:
...
**parameter `enable_numeric_scaling` - Масштабирование числовых признаков**

- **Что означает**: Включает автоматическое масштабирование числовых признаков
- **Зачем нужен**: Нормализует числовые признаки for лучшей performance
- **on умолчанию**: `True` (включено)
...
- **Что означает**: parameters for обработки временных данных
- **Зачем нужны**: Автоматически создают признаки из временных столбцов
- **Основные parameters**:
- **`enable_datetime_features`**: create временных признаков
- **`datetime_features`**: Типы временных признаков

**parameter `enable_datetime_features` - create временных признаков**

- **Что означает**: Включает create признаков из временных данных
...
- **Час**: Час из времени
- **minutesа**: minutesа из времени
- **Практические examples**:
- **Включить**: `enable_datetime_features=True`
- **Отключить**: `enable_datetime_features=False`
- **Когда использовать**:
- **Включить**: Когда есть временные data
- **Отключить**: Когда временные data not важны

**Практические examples Settings генерации признаков:**

```python
# example 1: Стандартная configuration
standard_features = {
...
# example 2: Быстрая configuration
quick_features = {
...
# example 3: Детальная configuration
Detailed_features = {
 'enable_text_special_features': True,
 'enable_text_ngram_features': True,
...
**Дополнительные parameters обработки текста:**

```python
# Расширенная configuration обработки текста
...
# Использование расширенной Settings
predictor.fit(
 train_data,
...
**Рекомендации on выбору параметров генерации признаков:**
...
"""Выбор параметров генерации признаков on basis типа данных"""

 if data_type == 'text_heavy':
# Текстовые data - акцент on текстовые признаки
 return {
 'enable_text_special_features': True,
 'enable_text_ngram_features': True,
 'text_ngram_range': (1, 3),
...
# Категориальные data - акцент on категориальные признаки
 return {
 'enable_text_special_features': False,
...
# Числовые data - акцент on числовые признаки
 return {
 'enable_text_special_features': False,
 'enable_text_ngram_features': False,
 'enable_categorical_encoding': True,
...
# Смешанные data - сбалансированная configuration
 return {
 'enable_text_special_features': True,
 'enable_text_ngram_features': True,
 'text_ngram_range': (1, 3),
...
print("configuration for текстовых данных:", text_config)
print("configuration for категориальных данных:", categorical_config)
print("configuration for числовых данных:", numeric_config)
print("configuration for смешанных данных:", mixed_config)
```
...
<img src="images/optimized/metrics_Detailed.png" alt="configuration метрик" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 7: configuration and configuration метрик оценки качества*

**Почему важна правильная configuration метрик?** Потому что metrics определяют, как оценивается качество модели:

- **Classification Metrics**: metrics for задач классификации (accuracy, precision, recall, F1, ROC-AUC)
- **Regression Metrics**: metrics for задач регрессии (RMSE, MAE, MAPE, R²)
- **Custom Metrics**: Кастомные metrics for специфических задач
- **Multi-Metric Evaluation**: Оценка on нескольким метрикам simultaneously
- **Metric Weighting**: Взвешивание метрик on важности
- **Threshold Optimization**: Оптимизация порогов for метрик
- **Metric Monitoring**: Monitoring метрик во время обучения
...
#### 🔧 Детальное describe Settings метрик

**parameter `eval_metric` - Метрика оценки**

- **Что означает**: Метрика for оценки качества модели
- **Зачем нужна**: Определяет, как измерять качество модели
- **on умолчанию**: `'auto'` (автоматический выбор)
- **Типы метрик**:
- **Строка**: Название встроенной metrics
- **List**: Несколько метрик
...
**Встроенные metrics for классификации:**

- **`'accuracy'`** - Точность (правильные предсказания / общее количество)
- **`'f1'`** - F1-мера (гармоническое среднее precision and recall)
- **`'f1_macro'`** - F1-мера with макро-усреднением
- **`'f1_micro'`** - F1-мера with микро-усреднением
- **`'f1_weighted'`** - F1-мера with взвешенным усреднением
...
- **`'log_loss'`** - Логарифмическая function потерь
- **`'balanced_accuracy'`** - Сбалансированная точность

**Встроенные metrics for регрессии:**
...
**parameter `Scorer` - Кастомная метрика**

- **Что означает**: Класс for создания кастомных метрик
- **Зачем нужен**: Позволяет создавать собственные metrics оценки
- **Основные parameters**:
- **`name`**: Название metrics
- **`score_func`**: function расчета metrics
- **`greater_is_better`**: Лучше ли большее значение
- **`needs_proba`**: Нужны ли вероятности
- **`needs_threshold`**: Нужен ли порог

**parameter `name` - Название metrics**

- **Что означает**: Уникальное название metrics
- **Зачем нужен**: Идентифицирует метрику in логах and результатах
- **on умолчанию**: `None` (автоматическое название)
- **Рекомендации**:
...
**parameter `score_func` - function расчета metrics**

- **Что означает**: function for расчета значения metrics
- **Зачем нужна**: Реализует Logsку расчета metrics
- **Требования к functions**:
- **Входные parameters**: `(y_true, y_pred)` or `(y_true, y_pred, y_pred_proba)`
- **Возвращаемое значение**: Числовое значение metrics
- **Обработка ошибок**: function должна обрабатывать ошибки
- **Практические examples**:
...
- **Метрика with вероятностями**: `score_func=lambda y_true, y_pred, y_pred_proba: roc_auc_score(y_true, y_pred_proba)`

**parameter `greater_is_better` - Направление оптимизации**

- **Что означает**: Указывает, лучше ли большее значение metrics
- **Зачем нужен**: Определяет направление оптимизации
- **on умолчанию**: `True` (большее значение лучше)
- **Практические examples**:
- **Точность**: `greater_is_better=True` (больше точность = лучше)
- **Ошибка**: `greater_is_better=False` (меньше ошибка = лучше)
- **Когда использовать**:
- **True**: for метрик, где большее значение лучше
- **False**: for метрик, где меньшее значение лучше

**parameter `needs_proba` - Нужны ли вероятности**

- **Что означает**: Указывает, нужны ли вероятности for расчета metrics
- **Зачем нужен**: Определяет, передавать ли вероятности in функцию
- **on умолчанию**: `False` (вероятности not нужны)
- **Практические examples**:
- **Точность**: `needs_proba=False` (нужны предсказания)
- **ROC AUC**: `needs_proba=True` (нужны вероятности)
- **Когда использовать**:
- **False**: for метрик, Workingющих with предсказаниями
- **True**: for метрик, Workingющих with вероятностями

**parameter `needs_threshold` - Нужен ли порог**

- **Что означает**: Указывает, нужен ли порог for расчета metrics
- **Зачем нужен**: Определяет, передавать ли порог in функцию
- **on умолчанию**: `False` (порог not нужен)
- **Практические examples**:
- **Точность**: `needs_threshold=False` (порог not нужен)
- **F1-мера**: `needs_threshold=True` (нужен порог)
- **Когда использовать**:
- **False**: for метрик, not требующих порога
- **True**: for метрик, требующих порога

**Практические examples кастомных метрик:**

```python
# example 1: Простая кастомная метрика
def custom_accuracy(y_true, y_pred):
"""Кастомная точность with дополнительной Logsкой"""
 from sklearn.metrics import accuracy_score

# Дополнительная Logsка
 if len(y_true) == 0:
 return 0.0
...
# example 2: Метрика with вероятностями
def custom_roc_auc(y_true, y_pred, y_pred_proba):
"""Кастомный ROC AUC with дополнительной обработкой"""
...
# example 3: Метрика with порогом
def custom_f1_with_threshold(y_true, y_pred, threshold=0.5):
"""Кастомная F1-мера with настраиваемым порогом"""
...
# Смешанные metrics (встроенные + кастомные)
predictor.fit(
 train_data,
...
**Детальное describe множественных метрик:**

**parameter `eval_metric` - List метрик**

- **Что означает**: List метрик for оценки качества модели
- **Зачем нужен**: Позволяет оценивать модель on нескольким критериям
- **on умолчанию**: `'auto'` (автоматический выбор)
- **Типы элементов списка**:
- **Строки**: Названия встроенных метрик
- **Scorer**: Кастомные metrics
- **Смешанные**: Комбинация встроенных and кастомных
- **Практические examples**:
- **Встроенные metrics**: `['accuracy', 'f1', 'roc_auc']`
- **Кастомные metrics**: `[custom_scorer1, custom_scorer2]`
- **Смешанные metrics**: `['accuracy', custom_scorer, 'roc_auc']`

**Стратегии выбора метрик:**

```python
# Стратегия 1: Базовые metrics
basic_metrics = ['accuracy', 'f1', 'roc_auc']

# Стратегия 2: Детальные metrics
Detailed_metrics = [
 'accuracy', 'f1', 'f1_macro', 'f1_micro', 'f1_weighted',
 'precision', 'recall', 'roc_auc', 'log_loss'
]

# Стратегия 3: Специализированные metrics
specialized_metrics = [
'f1_macro', # for несбалансированных данных
'roc_auc', # for ранжирования
'log_loss' # for вероятностей
]

# Стратегия 4: Кастомные metrics
custom_metrics = [
...
**Оптимизация on типу задачи:**

```python
# Классификация - стандартные metrics
classification_metrics = ['accuracy', 'f1', 'roc_auc']

# Классификация - несбалансированные data
...
**Monitoring множественных метрик:**

```python
# Callback for Monitoringа метрик
def metrics_monitor(epoch, Logs):
"""Monitoring метрик во время обучения"""
 print(f"Epoch {epoch}:")
...
"""Выбор метрик on basis типа задачи and характеристик данных"""
...
print("metrics for сбалансированной классификации:", balanced_classification)
print("metrics for несбалансированной классификации:", imbalanced_classification)
print("metrics for стандартной регрессии:", standard_regression)
print("metrics for процентной регрессии:", percentage_regression)
```

**Дополнительные возможности Settings метрик:**

```python
# configuration весов метрик
weighted_metrics = {
 'accuracy': 0.4,
 'f1': 0.3,
 'roc_auc': 0.3
...
# configuration порогов for метрик
threshold_metrics = {
 'f1': 0.5,
 'precision': 0.6,
 'recall': 0.4
...
# configuration усреднения for метрик
averaging_metrics = {
 'f1': 'macro',
 'precision': 'weighted',
 'recall': 'micro'
...
```python
# Обучение with весами метрик
predictor.fit(
 train_data,
 eval_metric=weighted_metrics
)

# Обучение with порогами метрик
predictor.fit(
 train_data,
 eval_metric=threshold_metrics
)

# Обучение with усреднением метрик
predictor.fit(
 train_data,
...
<img src="images/optimized/production_architecture.png" alt="Продакшен configuration" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 8: architecture and configuration for продакшена*

**Почему важна правильная продакшен configuration?** Потому что она обеспечивает стабильность and performance in реальных условиях:

- **Model Optimization**: Оптимизация модели for быстрого инференса
- **Resource Constraints**: Учет ограничений ресурсов in продакшене
- **Scalability**: Масштабируемость for больших нагрузок
- **Monitoring**: Monitoring performance and качества
- **Versioning**: Management версиями моделей
- **A/B testing**: Тестирование разных версий моделей
- **Rollback Strategy**: Стратегия Rollbackа при проблемах
...
### 🎯 Комплексная configuration

<img src="images/optimized/production_comparison.png" alt="Комплексная configuration" style="max-width: 100%; height: auto; display: block; margin: 20px auto;">
*Рисунок 9: comparison различных подходов к конфигурации*

**Почему важны examples комплексной конфигурации?** Потому что они показывают, как объединить все Settings for максимальной эффективности:

- **Hyperparameter Tuning**: Тонкая configuration параметров алгоритмов
- **Ensemble Configuration**: Оптимальная configuration ансамблей
- **Resource Management**: Эффективное Management ресурсами
- **Feature Engineering**: Продвинутая инженерия признаков
- **Validation Strategy**: Надежная стратегия validation
- **Metrics Selection**: Выбор подходящих метрик
- **Production Optimization**: Оптимизация for продакшена
