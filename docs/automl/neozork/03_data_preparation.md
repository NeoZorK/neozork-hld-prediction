...
**Goal:** Научиться правильно подготавливать and очищать финансовые данные for ML-моделей.
...
- **Конкурентное преимущество:** Качественные данные дают преимущество on рынке
...
**Теория:** Финансовые данные имеют специфическую структуру and характеристики, которые необходимо учитывать при подготовке for ML-моделей. Понимание типов данных критично for создания эффективных признаков and предотвращения ошибок.

### 1. OHLCV данные

**Теория:** OHLCV (Open, High, Low, Close, Volume) - это стандартный формат финансовых данных, который содержит основную информацию о торговых сессиях. Эти данные являются основой for большинства финансовых ML-моделей.

**Почему OHLCV данные важны:**
- **Полнота информации:** Содержат всю необходимую информацию о торговой сессии
- **Стандартизация:** Универсальный формат, поддерживаемый всеми platformми
- **Историческая совместимость:** Позволяют работать with историческими данными
...
**Теория:** Процентные изменения (returns) являются более стабильными for ML-моделей, чем абсолютные цены. Они нормализуют данные and делают их более подходящими for машинного обучения.
...
- **Ошибки in вычислениях:** Многие алгоритмы not могут работать with пропущенными значениями
- **Искажение статистики:** Пропуски искажают статистические свойства данных
- **Снижение качества модели:** Модели, обученные on данных with пропусками, работают хуже
...
def handle_missing_values(df):
 ...
 """
 # Forward fill for цен - используем последнее известное значение
...
- **Преимущества:** Работает with многомерными данными
...
def create_technical_indicators(df):
 ...
 # SMA сглаживает ценовые данные and показывает общий тренд
...
- **Автокорреляция:** Финансовые данные имеют сильную автокорреляцию - текущие значения зависят from предыдущих
...
- Работает with большинством алгоритмов
- Стандартизированный подход
- Хорошо работает with нормальными распределениями

**Недостатки StandardScaler:**
- Чувствительность к выбросам
- Может not работать with сильно скошенными распределениями
...
def standardize_data(df):
 ...
 Стандартизация преобразует данные так, чтобы они имели среднее 0 and стандартное отклонение 1.

 Принцип работы:
 1. Вычисляет среднее and стандартное отклонение for каждого приsign
 2. Применяет формулу: z = (x - mean) / std
 3. Возвращает нормализованные данные and объект scaler
...
- **Совместимость:** Работает with алгоритмами, чувствительными к масштабу
...
- **Процентные данные:** Когда данные уже in процентном формате
...
- Хорошо работает with ограниченными данными

**Недостатки MinMaxScaler:**
- Чувствительность к выбросам
- Может сжимать данные in узкий диапазон
- not работает with неограниченными распределениями
- Требует сохранения параметров
- Может искажать данные with выбросами
...
- **Портфельное управление:** Помогает in распределении активов
...
def validate_data_quality(df):
 ...
 # Проверяем on дубликаты временных меток
 time_consistency['duplicate_timestamps'] = df.index.duplicated().sum() / len(df)

 # Проверяем on пропуски во времени
 if len(df) > 1:
...
**Теория:** check стабильности данных - это процесс анализа временных рядов on предмет стационарности, стабильности статистических свойств and отсутствия структурных изменений. in финансовых данных стабильность критически важна for надежности ML-моделей.
...
def check_data_stability(df):
 ...
 # Проверяем автокорреляцию in остатках (если есть тренд)
 if len(df) > 10:
 try:
 ...
 # Разделяем данные пополам and сравниваем статистики
 if len(df) > 20:
 mid_point = len(df) // 2
...
def save_data_parquet(df, filename):
 ...
 - Сохраняет типы данных and метаданные
...
- **Иерархическая структура:** Группы and подгруппы
- **Метаданные:** Атрибуты for описания данных
...
def save_data_hdf5(df, filename):
 ...
 - Сохраняет метаданные and атрибуты
