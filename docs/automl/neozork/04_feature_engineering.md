...
**Цель:** Научиться создавать эффективные признаки for ML-моделей in финансовых данных.
...
**Теория:** for демонстрации инженерии признаков нам нужны реалистичные финансовые данные. Мы создадим синтетические данные, которые имитируют реальные рыночные условия, включая тренды, волатильность, сезонность and шум.
...
## Специализированные признаки for торговли
...
## Признаки for разных таймфреймов

**Теория:** Мультитаймфреймовый анализ является мощным инструментом in техническом анализе and машинном обучении. Он позволяет учитывать различные временные горизонты and выявлять паттерны, которые видны только on определенных таймфреймах.

**Почему мультитаймфреймовый анализ важен:**
- **Полная картина:** Разные таймфреймы показывают разные аспекты рынка
- **Подтверждение сигналов:** Сигналы on разных таймфреймах подтверждают друг друга
...
def create_multitimeframe_features(df, timeframes=['1H', '4H', '1D']):
 """
 create мультитаймфреймовых признаков
 ...
 parameters:
 - df: DataFrame with OHLCV данными
 - timeframes: список таймфреймов for analysis

 Возвращает:
 - DataFrame with мультитаймфреймовыми приsignми
 """
 print("🔄 create мультитаймфреймовых признаков...")
 ...
 # Resample for разных таймфреймов
 ...
 print(f" ⚠️ Недостаточно данных for таймфрейма {tf}")
 continue

 # 1. Технические индикаторы for каждого таймфрейма
 ...
 # 6. Синхронизация with исходным таймфреймом
 ...
 print(f"✅ Создано мультитаймфреймовых признаков for {len(timeframes)} таймфреймов")

 return result_df

# Демонстрация создания мультитаймфреймовых признаков
print("\n" + "="*60)
print("⏰ ДЕМОНСТРАЦИЯ: create мультитаймфреймовых признаков")
print("="*60)

# create мультитаймфреймовых признаков
df_multitimeframe = create_multitimeframe_features(sample_data.copy(), ['1D', '1W'])

# Показ статистики on мультитаймфреймовым приsignм
print(f"\n📊 Статистика on мультитаймфреймовым приsignм:")
...
print(f"\n📋 examples мультитаймфреймовых признаков (последние 5 дней):")
